...
def task1(config):
    ...
    for num, P1 in shp_gt.items():
        if num in shp_pd:
            ...
            P2 = []
        # remove overlapping or invalid polygons.
        if len(P2) > 0:
            polys = [Polygon(p) for p in P2]
            # remove invalid ones
            polys = [p for p in polys if p.is_valid]
            # remove overlapping ones randomly.
            sel = [0]
...
def task3(config):
    ...
    with open(osp.join(config.gtpath, 'Task3', 'GroundData',
        ...
        for l in lines[1:]:
            ...
            all_gt_spid.append(speciesid)
        assert len(gt_crownid2spid) > 0
        all_gt_spid = list(set(all_gt_spid))
        spid2ind = {s: k for k, s in enumerate(all_gt_spid)}
    ...
    with open(osp.join(config.income, 'species_id_subm.csv'), 'r') as fp:
        ...
        spidlist = sorted(all_gt_spid)
        num_sp = len(all_gt_spid)
        ...
        for l in lines[1:]:
            ...
            if crownid not in gt_crownid2spid:
                print colored('[Error] crownid in Task3 is invalid.' % crownid)
                exit(0)
            gt_spid = gt_crownid2spid[crownid]
            max_prob = -1
            max_spid = None
            cross_entropy_matched = False
            for k, prob in enumerate(elems[1:]):
                ...
                prob = float(prob)
                CM[spid2ind[gt_spid], spid2ind[pd_spid]] += prob
                if pd_spid == gt_spid:
                    cross_entropy.append(-np.log(0.001 + prob))
                    cross_entropy_matched = True
            if max_spid == gt_spid:
                rank_1 += 1
            if cross_entropy_matched == False:
                # testing sample classified to gt class with prob 0.
                cross_entropy.append(-np.log(0.001))
